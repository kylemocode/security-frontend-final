                ...
                <LeftCardTitle>
                    ...
                    <div style={{width:'100%',display:'flex',flexDirection:'column',alignItems:'center',marginTop:'-50px'}}>
                        <div style={{display:'flex',alignItems:'flex-start',marginTop:'15px'}}>
                            <i class="fas fa-laptop" style={{marginTop:'4px',marginRight:'10px'}}></i>
                            <p>{props.computerInfo.deviceName}</p>
                        </div>
                        <div style={{display:'flex'}}>
                            <p style={{marginTop:'-15px'}}>最近一次掃描分數:   </p>
                            <ScoreIcon style={{marginTop:'-15px'}} bgc={colorPicker(props.computerInfo.latest_scan_score)}>{props.computerInfo.latest_scan_score}</ScoreIcon>
                        </div>
                        <div style={{display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center'}}>
                            <p style={{marginTop:'-15px'}}>最近一次安全狀態:   {props.computerInfo.last_scan_status}</p>
                            <TrafficLightContainer>
                                <TrafficLight bgc="#31f745" light={props.computerInfo.latest_scan_score < 4 && true}/>
                                <TrafficLight bgc="#f7b539" light={props.computerInfo.latest_scan_score < 7 && props.computerInfo.latest_scan_score > 3 && true}/>
                                <TrafficLight bgc="#f53022" light={props.computerInfo.latest_scan_score < 10 && props.computerInfo.latest_scan_score > 6 && true}/>
                            </TrafficLightContainer>
                        </div>
                    </div>
