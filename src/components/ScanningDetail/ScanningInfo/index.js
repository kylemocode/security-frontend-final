...
import {
  ...
  LeftCardTitle,
  LeftCardBody
} from "../../ScanningHistory/ComputerInfo/style";
...
const ScanningInfo = props => {
  const statusResolver = status => {
    if (
      status.normalOption &&
      !status.advancedOption &&
      !status.customizedOption
    ) {
      return "一般";
    }
    if (
        ...
        <LeftCardTitle>
          ...
          >
            ...
            >
              ...
              <p>
                Scan ID:{" "}
                <span style={{ marginLeft: "10px" }}>{props.scanId}</span>
              </p>
            </div>
            ...
            >
              <p style={{ marginTop: "-15px" }}>
                安全狀態: {statusPicker(props.score)}
              </p>
              <TrafficLightContainer>
                <TrafficLight bgc="#31f745" light={props.score < 4 && true} />
                <TrafficLight
                  bgc="#f7b539"
                  light={props.score < 7 && props.score > 3 && true}
                />
                <TrafficLight
                  bgc="#f53022"
                  light={props.score < 10 && props.core > 6 && true}
                />
              </TrafficLightContainer>
            </div>
          </div>
